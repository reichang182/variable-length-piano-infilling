...
def construct_dict(save_dict_path):
    ...
    for etype in ['Tempo', 'Bar', 'Position', 'Pitch', 'Duration', 'Velocity']:
        ...
        e2w['%s <BLK>' % etype] = count
        count += 1
        e2w['%s <SEP>' % etype] = count
        count += 1
        e2w['%s <ANS>' % etype] = count
        count += 1
...
def load_tuple_event_2track(files=None):
    data = {'train':[], 'evaluation': []}
    if files == None:
        files = glob.glob(os.path.join(data_path, '*.midi'))
    for data_segment in ['train']:
        for midifile in files:
            all_events = extract_tuple_events_2track(midifile)
            piano_events, melody_events = group_by_bar(all_events[0]), group_by_bar(all_events[1])   # shape of events: [n_bars, n_notes_per_bar]

            # del notes appear in both piano and melody track
            max_bar = min(len(piano_events), len(melody_events))
            for bar_idx in range(max_bar):
                for i_piano in reversed(range(len(piano_events[bar_idx]))):
                    for note_melody in melody_events[bar_idx]:
                        if event_match(piano_events[bar_idx][i_piano], note_melody):
                            piano_events[bar_idx].pop(i_piano)
                            break
            # TODO song too long?
            data[data_segment].append([piano_events, melody_events])
...
def prepare_data_for_training(data_file, e2w=None, w2e=None, is_train=True, n_step_bars=16, max_len=512):
    ...
    # shape of data or data: [n_midi, n_bars, n_notes, 5]. Last dimension is 5 since it contains `bar, position, pitch, duration and velocity`
    x_lens = []
    xs = []
    song_idx = []
    for song_id, midi in enumerate(data):
        for start in range(0, len(midi) - n_bars_per_x + 1, n_step_bars):
            ...
            if is_train:
                ...
                if len(x) <= max_len -3:    # TODO: >300 for using unseen data. Change to add all?
                    xs.append(x)
                    x_lens.append(len(x))
